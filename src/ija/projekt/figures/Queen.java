...
public class Queen extends Figure {
    ...
    public boolean canMove(Position destination) {
        ...
        if(!this.getPosition().isStraightOf(destination) && !this.getPosition().isDiagonalOf(destination)){
            return false;
        }
    ...
    public boolean canCapture() {
        for(int dirx = -1; dirx <= 1; dirx += 2) {
            for(int diry = -1; diry <= 1; diry += 2) {
                ...
                for(Position act = this.getPosition().nextPosition(2*dirx, 0); act != null; act = act.nextPosition(dirx, 0)) {
                    if (this.canCapture(act)) {
                        return true;
                    }
                }
                for(Position act = this.getPosition().nextPosition(0, 2 * diry); act != null; act = act.nextPosition(0, diry)) {
                    if (this.canCapture(act)) {
                        return true;
                    }
                }
    ...
    public boolean canCapture(Position destination) {
        if (destination == null) {
            return false;
        }else if(!this.getPosition().isDiagonalOf(destination) && !(this.getPosition().isStraightOf(destination))){
            return false;
        }else {
            int xinc = this.getPosition().x;
            int yinc = this.getPosition().y;
